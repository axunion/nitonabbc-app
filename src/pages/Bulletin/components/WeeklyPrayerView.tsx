...
import styles from "./WeeklyPrayerView.module.css";
    ...
    <Show when={hasAny()}>
      <section class={sectionStyles.section}>
        ...
        <div class={sectionStyles.sectionBody}>
          <dl class={styles.prayerList}>
            <For each={entries()}>
              {(entry) => (
                <Show when={entry.texts.some((t) => t.trim())}>
                  <dt class={styles.day}>{entry.label}</dt>
                  <dd class={styles.texts}>
                    <ul class={styles.textList}>
                      <For each={entry.texts.filter((t) => t.trim())}>
                        {(text) => <li class={styles.textItem}>{text}</li>}
                      </For>
                    </ul>
                  </dd>
                </Show>
